...
const _ADM_AGE_MIN = 5;
const _ADM_AGE_MAX = 18;
...
function _calcAge(birthDateStr) {
  ...
  const label  = years + ' سنة' + (months > 0 ? ' و' + months + ' شهر' : '');
  let status, note;
  if (years < _ADM_AGE_MIN) {
    status = 'invalid';
    note   = '⚠️ أصغر من الحد الأدنى (' + _ADM_AGE_MIN + ' سنوات)';
  } else if (years > _ADM_AGE_MAX) {
    status = 'ok';
    note   = '';
  } else if (years === _ADM_AGE_MIN || years === _ADM_AGE_MAX) {
    status = 'warn';
    note   = 'على الحد — يُرجى التحقق من الإثبات';
  } else {
    status = 'ok';
    note   = '';
  }
...
function _ageBadgeHTML(birthDateStr) {
  const age = _calcAge(birthDateStr);
  if (!age) return '';
  const colors = {
  ...
  const style = colors[age.status] || colors.ok;
  const noteHTML = age.note
    ? `<span style="font-size:11px;color:${age.status==='invalid'?'#b91c1c':age.status==='warn'?'#a16207':'#64748b'};margin-right:6px">${age.note}</span>`
    : '';
  return `<span style="display:inline-flex;align-items:center;gap:6px;flex-wrap:wrap">
    <span style="${style};font-size:12px;font-weight:700;padding:2px 10px;border-radius:20px;display:inline-block">${age.label}</span>
    ${noteHTML}
  </span>`;
}
      ...
      <div class="adm-req-body">
        <div class="adm-req-row">
          ...
          <span style="display:flex;align-items:center;gap:10px;flex-wrap:wrap">
            ...
            ${r.birthDate ? _ageBadgeHTML(r.birthDate) : ''}
          </span>
        </div>
        <div class="adm-req-row">
