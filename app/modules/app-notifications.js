...
async function refreshNotifBadge() {
  ...
  const admCount = typeof _pendingAdmCount !== 'undefined' ? _pendingAdmCount : 0;
...
async function renderNotifPanel() {
  ...
  // ── Admission pending requests card ──
  if (admCount > 0) {
    html += `
      <div class="notif-card" style="border-right:3px solid #f59e0b;cursor:pointer" onclick="closeNotifPanel();navigate('admissions')">
        <div class="notif-card-icon" style="font-size:18px">📋</div>
        <div class="notif-card-body">
          <div class="notif-card-title">لديك ${admCount} طلب قبول في الانتظار</div>
          <div class="notif-card-sub">اضغط للذهاب إلى صفحة طلبات القبول</div>
        </div>
      </div>`;
  }
