...
const ROLE_PAGES = {
  admin:     ['dashboard','attendance','students','admissions','quran','classes','teachers','checkin','holidays','calendar','reports','sync','settings','whatsapp','accounts'],
  moderator: ['dashboard','attendance','students','admissions','classes','teachers','checkin','holidays','calendar','reports','settings'],
    ...
    if (data.valid) {
      ...
      (function() {
        ...
        function connectEvents() {
          ...
          _es.addEventListener('newAdmission', function(e) {
            try {
              var d = JSON.parse(e.data);
              if (typeof window._onSSEAdmission === 'function') {
                window._onSSEAdmission(d);
              } else {
                // fallback if admission module not yet wired
                if (typeof _pendingAdmCount !== 'undefined') _pendingAdmCount++;
                if (typeof _updateAdmNavBadge === 'function') _updateAdmNavBadge();
