...
// مكتبة تقويم أم القرى الرسمي (حسابات فلكية دقيقة مطابقة للتقويم الرسمي السعودي)
// تثبيت: npm install @umalqura/core
let UmAlQuraLib;
try {
  const _uq = require('@umalqura/core');
  UmAlQuraLib = _uq.default || _uq;
} catch(e) {
  UmAlQuraLib = null;
  console.error('[hijri] تحذير: حزمة @umalqura/core غير مثبتة — سيُستخدم تقويم هجري حسابي تقريبي بدلاً من تقويم أم القرى الرسمي (قد يختلف يوماً أو يومين). للتثبيت: npm install @umalqura/core');
}
...
// ── تحويل التاريخ الميلادي ↔ هجري (تقويم أم القرى الرسمي عبر @umalqura/core) ──
// احتياطي: تقويم هجري حسابي تقريبي (تجاري تقليدي، دورة 30 سنة) يُستخدم فقط
// إن لم تكن حزمة @umalqura/core مثبتة — قد يختلف يوماً أو يومين عن أم القرى الرسمي.
...
function _fromHijriFallback(hy, hm, hd) {
...
function toHijri(dateStr) {
  ...
  if (UmAlQuraLib) {
    ...
    const h = UmAlQuraLib(new Date(Date.UTC(gy, gm-1, gd, 12, 0, 0)));
...
function fromHijri(hy, hm, hd) {
  if (UmAlQuraLib) {
    const inst = UmAlQuraLib(hy, hm, hd);
    return inst.date.toISOString().slice(0,10);
  }
  return _fromHijriFallback(hy, hm, hd);
}
...
app.get('/api/print/quran/class/:classId', (req, res) => {
  ...
  const hijriNow = formatHijri(nowDate());
